...
const copyColumn = (tab, columnIndex, insertIndex, rowIndex = 0)=>{
    ...
    if (rowIndex >= rows.length) return null;
        ...
        const newCell = rowIndex === 0 ? document.createElement("th") : document.createElement("td");
        newCell.textContent = row.children[columnIndex].textContent;
        if (insertIndex >= row.children.length) row.appendChild(newCell);
        else row.insertBefore(newCell, row.children[insertIndex]);
